...
try:
    VERIFY_GATEWAY = app.config.from_envvar('VERIFY_GATEWAY')  
    PUSHER_GATEWAY = app.config.from_envvar('PUSHER_GATEWAY')  
except Exception as e:
    VERIFY_GATEWAY = 'http://10.7.7.22:9090'
    PUSHER_GATEWAY = 'http://10.7.7.22/api/passport/push/'
...
def push():
    ...
    # print data
    resp = requests.post(url=PUSHER_GATEWAY, json=request.form['data'])
    return resp.content
